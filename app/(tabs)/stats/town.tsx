...
export default function Page() {
  ...
  const totalItems = data.towns.length;
  ...
  const currentItems = data.towns.slice(startIndex, endIndex);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <StyledView key={item.name} className="flex-row justify-between p-2 border-b border-slate-600">
      ...
      <StyledText className="text-right text-white">{item.mayor}</StyledText>
    </StyledView>
  );

  return (
    <StyledView className="flex-1 bg-slate-800 p-8">
      <StyledText className="text-2xl font-bold text-white mb-4">Towns</StyledText>
      <StyledView className="flex-row justify-between p-2 border-b-2 border-white">
        <StyledText className="font-bold text-white">Town</StyledText>
        <StyledText className="font-bold text-white">Mayor</StyledText>
      </StyledView>
      <FlatList
        data={currentItems}
