...
export default function Page() {
  ...
  const totalItems = data.nations.length;
  ...
  const currentItems = data.nations.slice(startIndex, endIndex);

  const renderItem = ({ item }: { item: any }) => (
    <StyledView key={item.name} className="flex-row justify-between p-2 border-b border-slate-600">
      <StyledText className="text-left text-white">{item.name}</StyledText>
      <StyledText className="text-right text-white">{item.capital}</StyledText>
    </StyledView>
  );

  return (
    <StyledView className="flex-1 bg-slate-800 p-8">
      <StyledText className="text-2xl font-bold text-white mb-4">Nations</StyledText>
      <StyledView className="flex-row justify-between p-2 border-b-2 border-white">
        <StyledText className="font-bold text-white">Nation</StyledText>
        <StyledText className="font-bold text-white">Capital</StyledText>
      </StyledView>
      <FlatList
        data={currentItems}
